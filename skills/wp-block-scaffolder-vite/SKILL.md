...
- **Register the SCSS in `vite.config.js` (Critical — do not skip)**:
  - Add `'blocks/{slug}': './scss/blocks/{slug}.scss'` to the `rollupOptions.input` object in `vite.config.js`
...
- `.claude/skills/wp-block-scaffolder-vite/docs/block-scaffolding.reference.md`
...
#### vite.config.js (Required — never skip)

Every new block SCSS file **must** be added to the `rollupOptions.input` object in `vite.config.js` or it will never compile. Add the entry alongside the other block entries:

```js
'blocks/{slug}': './scss/blocks/{slug}.scss',
```

Example for a `quick-links` block:

```js
'blocks/quick-links': './scss/blocks/quick-links.scss',
```

If the block also has a JavaScript enhancement file, add a second entry:

```js
'blocks/quick-links-js': './js/blocks/quick-links.js',
```

Failure to add these entries means the block has no styles (or scripts) in production — it is as critical as creating the files themselves.
...
- `.claude/skills/wp-block-scaffolder-vite/docs/styling-tokens.reference.md`
...
- `.claude/skills/wp-block-scaffolder-vite/docs/button-system.reference.md`
...
- `.claude/skills/wp-block-scaffolder-vite/docs/focuspoint.reference.md`
...
- `.claude/skills/wp-block-scaffolder-vite/docs/acf-json.reference.md`
...
- [ ] `vite.config.js` — `'blocks/{slug}': './scss/blocks/{slug}.scss'` entry added to `rollupOptions.input`
